...
As testando grows and the authentication method changes, only these tests
should be updated.

"""

from testando.tests import TestController
...
class TestAuthentication(TestController):
    ...
    def test_forced_login(self):
        ...
        assert resp.location.startswith('http://localhost:3005/login')
        ...
        assert post_login.location.startswith('http://localhost:3005/post_login')
        initial_page = post_login.follow(status=302)
        assert 'authtkt' in initial_page.request.cookies, \
               "Session cookie wasn't defined: %s" % initial_page.request.cookies
        assert initial_page.location.startswith('http://localhost:3005/secc/'), \
    ...
    def test_voluntary_login(self):
        ...
        assert home_page.location == 'http://localhost:3005/'

    def test_logout(self):
        """Logouts must work correctly"""
        # Logging in voluntarily the quick way:
        resp = self.app.get('/login_handler?login=admin&password=pass',
                            status=302)
        ...
        assert resp.location.startswith('http://localhost:3005/post_logout')
        ...
        assert home_page.location == 'http://localhost:3005/', home_page.location
